...
void setup() { 
  ...
  table = loadTable("crimerateSmaller.csv", "csv");
  ...
  for(int j = 0; j < dataCols; j++) {
    labels[j] = table.getString(0, j+1);
  }
  
  for(int i = 0; i < dataRows; i++) { 
    allLines[i] = new lineInfo();
    allLines[i].xLabel = table.getString(i+1, 0);
    ...
    for(int j = 0; j < dataCols; j++) {
      //println(i + " " + j);
      allLines[i].vals[j] = table.getFloat(i+1, j+1);
      array[i][j] = table.getFloat(i+1, j+1);
